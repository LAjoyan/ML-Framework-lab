...
class KaggleCifar10Dataset(Dataset):
    def __init__(
        ...
        if num_images is None:
            raise ValueError(
                "num_images must be provided (e.g., 50000 for train, 300000 for test)."
            )
        ...
        if labels_csv_path is not None:
            ...
            self.labels = np.array(
                [self.class_to_idx[name] for name in label_names], dtype=np.int64
            )
...
def get_dataloaders(config: dict):
    ...
    full_train_ds = KaggleCifar10Dataset(
